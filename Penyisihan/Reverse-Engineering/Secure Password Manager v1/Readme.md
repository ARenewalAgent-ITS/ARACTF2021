...
def inv_shift_rows(s):
	...
	s[1][2], s[2][2], s[3][2], s[0][2] = s[3][2], s[0][2], s[1][2], s[2][2]
	s[1][3], s[2][3], s[3][3], s[0][3] = s[2][3], s[3][3], s[0][3], s[1][3]
	return s
...
for seed in range(0,100,7):
	try:
    	...
    	key=[]
    	for i in flag:
        	key.append(model[i].as_long())
    	master_key = key[-16:][::-1]
    	tmp="".join(map(chr,master_key))
    	print(tmp,len(tmp))
    	encrypted=[0xe4,0xff,0x9b,0xc8,0x25 ,0x92 ,0x55 ,0x0 ,0xd4 ,0x85 ,0x6c ,0xca  ,0x6 ,0x9b ,0x4e  ,0xb]

    	flag=decrypt(encrypted,master_key)
    	fix=""
    	for i in flag:
        	fix+=chr(i^seed)
    	print("ara2021{"+fix[::-1]+"}")
